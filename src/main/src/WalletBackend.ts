...
export class WalletBackend {
  ...
  private initHandlers(): void {
    ...
    ipcMain.handle(
      "deleteWallet",
      new DeleteWalletHandler(this.walletService).handle,
    );
    ipcMain.handle(
      "getAllWallets",
      new GetAllWalletsHandler(this.walletService).handle,
    );
    ipcMain.handle("selectWallet", new SelectWallet(this.walletService).handle);
    ipcMain.handle(
      "getWalletBalance",
      new GetWalletBalance(this.walletService).handle,
    );
    ipcMain.handle(
      "getAddresses",
      new GetWalletAddressesHandler(this.walletService).handle,
    );
    ipcMain.handle(
      "getReceiveAddress",
      new GetReceiveAddressHandler(this.walletService).handle,
    );
    ipcMain.handle(
      "getStatus",
    ...
    ipcMain.handle(
      "getTransactions",
      new GetTransactionsHandler(this.walletService).handle,
    );
    ipcMain.handle("getBalance", new GetBalance(this.walletService).handle);
    ipcMain.handle(
      "getTransactionByHash",
      new GetTransactionByHashHandler(this.walletService).handle,
    );
    ipcMain.handle(
      "getIdentities",
      new GetIdentitiesHandler(this.walletService).handle,
    );
    ipcMain.handle(
      "getIdentityBalance",
      new GetIdentityBalance(this.walletService).handle,
    );
    ipcMain.handle(
      "getIdentityNonce",
      new GetIdentityNonce(this.walletService).handle,
    );
    ipcMain.handle(
      "getPlatformAddresses",
      new GetPlatformAddressesHandler(this.walletService).handle,
    );
    ipcMain.handle(
      "getBlockByHash",
      new GetBlockByHash(this.walletService).handle,
    );
    ipcMain.handle(
      "setAddressLabel",
      new SetAddressLabel(this.walletService).handle,
    );
    ipcMain.handle(
      "setWalletLabel",
      new SetWalletLabel(this.walletService).handle,
    );
    ipcMain.handle(
      "sendTransaction",
      new SendTransactionHandler(this.walletService).handle,
    );
    ipcMain.handle(
      "sendPlatformTransfer",
      new SendPlatformTransferHandler(this.walletService).handle,
    );
    ipcMain.handle(
      "verifyWalletPassword",
      new VerifyWalletPasswordHandler(this.walletService).handle,
    );
    ipcMain.handle(
      "exportMnemonic",
      new ExportMnemonicHandler(this.walletService).handle,
    );
    ipcMain.handle(
      "getPreferences",
      new GetPreferencesHandler(this.applicationService).handle,
    );
    ipcMain.handle(
      "setLanguage",
      new SetLanguageHandler(this.applicationService).handle,
    );
    ipcMain.handle(
      "setFiatCurrency",
      new SetFiatCurrencyHandler(this.applicationService).handle,
    );
    ipcMain.handle(
      "setConnectionType",
      new SetConnectionTypeHandler(this.applicationService).handle,
    );
    ipcMain.handle(
      "resetPreferences",
      new ResetPreferencesHandler(this.applicationService).handle,
    );
    ipcMain.handle(
      "startWalletSync",
      new StartWalletSyncHandler(this.walletSyncService).handle,
    );
    ipcMain.handle(
      "stopWalletSync",
      new StopWalletSyncHandler(this.walletSyncService).handle,
    );
    ipcMain.handle(
      "resetWalletSync",
      new ResetWalletSyncHandler(this.walletSyncService).handle,
    );
    ipcMain.handle(
      "getUtxos",
      new GetUtxosHandler(this.walletSyncService).handle,
    );
    ipcMain.handle(
      "hasSyncProgress",
      new HasSyncProgressHandler(this.walletSyncService).handle,
    );
    ipcMain.handle(
      "broadcastTransaction",
      new BroadcastTransactionHandler(this.walletSyncService).handle,
    );
    ipcMain.handle(
      "getExchangeRates",
      new GetExchangeRatesHandler(this.ratesService).handle,
    );
    ipcMain.handle(
      "getContacts",
      new GetContactsHandler(this.contactService).handle,
    );
    ipcMain.handle(
      "addContact",
      new AddContactHandler(this.contactService).handle,
    );
    ipcMain.handle(
      "deleteContact",
      new DeleteContactHandler(this.contactService).handle,
    );
  }
  ...
  async start(): Promise<void> {
    ...
    const preferences = await Preferences.init(
      path.join(os.homedir(), HomeFolderName, PreferencesFilename),
    );

    const knex = getKnex(
      path.join(os.homedir(), HomeFolderName, StorageFilename),
    );
    ...
    const dashPlatformSDK = new DashPlatformSDK({
      network: "testnet",
      grpc: {
        dapiUrl: "https://68.67.122.24:1443/",
        poolLimit: 5,
      },
    });

    this.applicationService = new ApplicationService(preferences);
    this.walletSyncService = new WalletSyncService(
      walletDAO,
      addressDAO,
      transactionDAO,
    );
